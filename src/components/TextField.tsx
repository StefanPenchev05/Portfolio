...
const TextField: React.FC<TextFieldProps> = ({
  ...
  return (
    <div
      className={`flex flex-col space-y-4 ${fullWidth ? "w-full" : "w-1/2"} p-2`}
    >
      ...
      {textArea ? (
        ...
        <input
          ...
          onChange={(e) => setText(e.target.value)}
          placeholder={`Enter ${title.toLowerCase()}`}
          className="bg-[#191b1e] rounded-md h-14 p-3 text-light-text focus:outline-none focus:ring-2 focus:ring-[#61DAFB] transition duration-200 ease-in-out"
        />
      )}
    </div>
  );
